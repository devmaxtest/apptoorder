...
- **Hébergement** : Replit (dev + production), Hetzner (option self-hosted)
- **CDN/DNS** : Cloudflare → macommande.shop
- **SSL** : Let's Encrypt (renouvellement automatique)
- **Temps réel** : WebSocket (sync commandes, notifications)
- **Paiement** : Stripe (abonnements SaaS — starter, website_app, premium)
- **Caisse (POS)** : HubRise (LEO2, Zelty, Lightspeed, Tiller)
- **Monitoring** : MaxAI COBA (événements, alertes, business intelligence)
- **Email** : SMTP (prévu, vérifié dans health check)
- **Gestion financière** : MyBusiness via API COBA (comptabilité, RH, banque, caisse)
...
| 2 | `database:tables` | 19 tables requises existent |
...
| 7+ | `restaurant:[slug]` | Par restaurant : catégories, plats, photos, services, commandes, stats, owner, URLs |
...
| `https://macommande.shop/pro/sugumaillane/mybusiness` | MyBusiness SUGU (finance/RH) |
| `https://macommande.shop/pro/lagaudina/mybusiness` | MyBusiness La Gaudina (finance/RH) |
...
| `https://apptoorder.replit.app/123admin/monitoring` | Dashboard monitoring COBA |

### Endpoints API publics (à tester via HTTP GET)
| Endpoint | Description |
|---|---|
| `/api/ping` | Health check léger |
...
### Portail Client (`/:slug/client`)
- Inscription/connexion client par restaurant
- Menu interactif avec catégories, plats, options, formules
- Panier avec calcul côté serveur
- Commande livraison ou à emporter
- Suivi commande en temps réel (WebSocket)
- Historique commandes
- Programme de fidélité

### Portail Pro (`/pro/:slug`)
- Gestion du menu (catégories, plats, drag-and-drop)
- Options de plats et modificateurs
- Formules (menus du jour) avec slots configurables
- Gestion des commandes en temps réel
- Statistiques (commandes du jour, en attente, CA)
- Personnalisation visuelle (couleurs, logo, sections)
- Gestion des photos et galerie
- Gestion des horaires de service
- Gestion des clients (fidélité, édition, suppression)
- Génération factures PDF
- Intégration HubRise (connexion POS)

### MyBusiness (`/pro/:slug/mybusiness`)
Suite ERP complète pour la gestion financière, connectée à l'API COBA :
- **Dashboard** : synthèse financière, audit, statistiques fichiers, sauvegardes
- **Achats** : gestion fournisseurs, pièces jointes par ligne
- **Frais** : charges opérationnelles, catégorisation
- **Banque** : mouvements, prêts, import relevés (PDF/CSV), rapprochement bancaire
- **Caisse** : saisies, scan de tickets (caméra + OCR)
- **Gestion RH** : employés, bulletins de paie (import PDF, reparse), absences, dossiers employés

### Portail Admin (`/123admin`)
- CRUD complet restaurants (création, édition, suppression, logo)
- Gestion utilisateurs (rôles, édition profil)
- Vue globale commandes tous restaurants
- Configuration HubRise (HUBRISE_CLIENT_ID / HUBRISE_CLIENT_SECRET)
- Export PDF données
- Monitoring COBA (`/123admin/monitoring`)
...
| Nouveau restaurant détecté | INFO | Log + mise à jour URLs |
| Changement de schéma DB | INFO | Log + notification |
| HubRise déconnecté alors qu'il était connecté | ATTENTION | Notification intégration |
...
-- Abonnements actifs
SELECT name, slug, subscription_plan, subscription_status, subscription_price
FROM restaurants ORDER BY subscription_status

-- Intégrations HubRise actives
SELECT name, slug, hubrise_connected, hubrise_location_id
FROM restaurants WHERE hubrise_connected = true
```
...
- **MyBusiness** utilise l'API COBA externe (`VITE_COBA_API_URL`) avec clé `VITE_COBA_API_KEY` — les endpoints financiers ne transitent pas par le backend Express
- **HubRise** peut être configuré dynamiquement depuis le portail admin — les identifiants sont persistés dans `.env` et `process.env`
